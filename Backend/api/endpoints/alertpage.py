...
def getRequests(request):
    ...
    try:
        pending = Request.objects.filter(
            listing_id__in=Listing.objects.filter(user_id=user_id, status=0).values_list('id', flat=True), status=0)
    ...
    try:
        accepted = Request.objects.filter(
            listing_id__in=Listing.objects.filter(user_id=user_id, status=1).values_list('id', flat=True), status=1)
    except:
    ...
    for r in pending:
        pending_requests.append({
            ...
            "user_profile": r.requester.profile_url,
    ...
    return JsonResponse({'msg': 'Success!', 'success': True, "pending_requests": pending_requests,
                         "accepted_requests": accepted_requests}, safe=False)
